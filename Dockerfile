...
ARG USER_NAME="eric-getting-started-java-app"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} getting-started-java-app-app.jar
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar getting-started-java-app-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-getting-started-java-app-jsb \
